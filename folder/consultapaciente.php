<?php
  ...
  if(!isset($_SESSION['cargo']) || $_SESSION['cargo'] != 1){
    ...
    */
	...
	echo '<script>
	var pacienteGlobal='.$_GET['codpaci'].';
	</script>';
  }
...
<!--Fin Modal IMC -->
			...
			$db = $database->open();
			try{	
				$sql = "SELECT * FROM customers WHERE codpaci = '".$_GET['codpaci']."'";
				$sql1 = "SELECT * FROM `archivos_paciente` WHERE codpaci = '".$_GET['codpaci']."'";
		...
		?>
			...
			<form method="POST" enctype="multipart/form-data" action="guardarconsulta.php">
                <div class="row">
					<div class="col-3">
					...
					<div class="col-7">
						...
						<div class="row">
                    		...
                    		<textarea class="form-control" id="fisico" name="fisico" rows="5" required></textarea>
						</div>
						<div class="row form-group">
							<div class="col-3">
								<input type="number" class="form-control input-sm" id="peso" placeholder="Peso" name="pesop" required><p style="text-align:center;">KG</p>
							</div>	
							<div class="col-3">
								<input type="number" step="0.01" id="talla" class="form-control" placeholder="Talla" name="tallap" required><p style="text-align:center;">Metros</p>
							</div>	
							<div class="col-3">
								<input type="text" class="form-control"  id="imc" placeholder="I.M.C." name="imcp"  readonly ><p style="text-align:center;">KG/M2</p>
							</div>
							<div class="col-3">
							...
							<div class="col-3">
								<input type="number" class="form-control" placeholder="SAT O2" name="sat" required><p style="text-align:center;">SpO2</p>
							</div>		
							<div class="col-3">
								<input type="number" step="0.01" class="form-control" placeholder="Temp. (C°)" name="tempp" required><p style="text-align:center;">°C</p>
							</div>	
							<div class="col-3">
								<input type="number" class="form-control" placeholder="Pulso" name="pulsop" required><p style="text-align:center;">FREC. C</p>
							</div>	
							<div class="col-3">
								<input type="text" class="form-control" placeholder="Presión" name="presionp" required><p style="text-align:center;">mmHg</p>
							</div>	
							<div class="col-3">
								<input type="number" class="form-control" placeholder="FAT %" name="fat" required><p style="text-align:center;">FAT%</p>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<div class="col-6">
						...
						<div class="row">
							...
							<input type="text" autocomplete="off" class="form-control" id="diagnostico" placeholder="Diagnostico principal" name="diagpri" required>
							
						</div>
						<hr />
						<button type="button"  class="btn btn-info" id="addDignostico">Añadir Diagnostico</button>
						<hr />
						<div class="row">
							<label for="diagsec">Diagnostico secundario</label>
							<textarea class="form-control" name="diagsec" rows="2"></textarea>
						</div>
					...
					<div class="input-group mb-3">
							<input type="text" id="examenes[] " value="" name="examenes[]" class="form-control" >
				...
				<div class="row">
					<div class="col-6">
						...
						<textarea class="form-control" name="indic" rows="4" placeholder="Indicaciones generales" required></textarea>
						<hr/>
						<div class="col-6"> 
	...
	<script>
		$( document ).ready(function() {
			//BUSCAR DIAGNOSTICO EN TIEMPO REAL CON JQUERY UI
$("#diagnostico").autocomplete({

source: "../assets/ajax/listarDiagnostico.php",
minLength: 2,
select: function (event, ui) {
	event.preventDefault();
	if (ui.item.diagnostico == "0") {
		$("#mensajecliente").css('display', 'block');
	} else {
		$('#diagnostico').val(ui.item.diagnostico);
			...
			$.getJSON("../model/modelAntecedents.php?codpaci="+pacienteGlobal, function(result){
			...
			$("#addDignostico").click(function () {
            	...
            	formData.append ('diag', diagnostico);
				$.ajax({
					type:"POST",
					url:"guardarDiagnostico.php",
					data:formData,
					contentType: false,
        			cache: false,
        			processData:false,

			...
			<div class="content">
				<div class="page-inner">
					...
					<div class="row">
						...
						<?php  } elseif($va['estado']==2){
							?> 
							<form  method="get" action="javascript:enEspera('<?php echo $va['codcit']; ?>')">
...
//BUSCAR CLIENTE EN TIEMPO REAL CON JQUERY UI
$("#nombrePaciente").autocomplete({

source: "../assets/ajax/listarPacientes.php",
minLength: 2,
select: function (event, ui) {
	event.preventDefault();
	if (ui.item.id_paciente == "0") {
		$("#mensajecliente").css('display', 'block');
	} else {
		$('#idPaciente').val(ui.item.id_paciente);
		$('#nombrePaciente').val(ui.item.nombre_paciente);
	}

}
});

$("#nombrePaciente").on("keydown", function (event) {
...
function enEspera(codcit)
{
	var id=codcit;
	$.ajax({
        type:"GET",
		url:"../assets/ajax/editar_estado_espera_cita.php?id="+id,
    }).done(function(data){
        window.location.href ='../folder/appointment.php';
    })
